...
from KBQA.appB.transformer_architectures.kb.common import set_requires_grad
from KBQA.appB.transformer_architectures.kb.entity_linking import BaseEntityDisambiguator
from KBQA.appB.transformer_architectures.kb.entity_linking import EntityLinkingBase
...
from KBQA.appB.transformer_architectures.kb.metrics import MeanReciprocalRank
from KBQA.appB.transformer_architectures.kb.span_attention_layer import SpanAttentionLayer
...
class BertPretrainedMaskedLM(BertPretrainedMetricsLoss):
    ...
    def forward(
        self, tokens, segment_ids, lm_label_ids=None, next_sentence_label=None, **kwargs
    ):
        mask = tokens["tokens"] > 0
        contextual_embeddings, pooled_output = self.bert.bert(
            tokens["tokens"], segment_ids, mask, output_all_encoded_layers=False
        )
