...
class BertTokenizerAndCandidateGenerator(Registrable):
    ...
    def _word_to_word_pieces(self, word):
        if self.do_lowercase and word not in list(self.bert_tokenizer.tokenizer.special_tokens_map.values()):
            word = word.lower()
        return self.bert_tokenizer.tokenize(word)[1:-1] #exclude [CLS] and [SEP] at beginning and end
        ...
        for key, entity_candidates in tokens_and_candidates['candidates'].items():
            ...
            candidate_fields = {
                ...
                "candidate_spans": ListField(
                    [SpanField(span[0], span[1], fields['tokens']) for span in entity_candidates['candidate_spans']]
                ),
