...
This file is copied from \"preprocess.py\" in the fairseq v0.8.0
and modified not to append the EOS tokens to source sentences.
"""
...
def main(args):
    ...
    def make_binary_dataset(
        vocab, input_prefix, output_prefix, lang, num_workers, append_eos=True
    ):
    ...
    def make_dataset(
        vocab, input_prefix, output_prefix, lang, num_workers=1, append_eos=True
    ):
    ...
    make_all(args.source_lang, src_dict, append_eos=False)
    if target:
...
def binarize(args, filename, vocab, output_prefix, lang, offset, end, append_eos):
    ds = indexed_dataset.make_builder(
        dataset_dest_file(args, output_prefix, lang, "bin"),
        impl=args.dataset_impl,
        vocab_size=len(vocab),
