...
from KBQA.appB.summarizers.utils import build_sparql_query_for_one_hop_predicates
...
def ask_for_entities(question: str, *, confidence: float = 0.8) -> List[URIRef]:
    ...
    """
    entities = list()
    entities += entity_recognition_dbspotlight(question, confidence=confidence)
    entities_falcon, _ = entity_relation_recognition(question)

    entities += entities_falcon
...
def query_dbpedia_for_all_entities(
    ...
    for pred in list_of_predicates:
        if len(pred[0]) != 2:
            one_hop_preds.append(pred)
        elif len(pred[0]) == 2:
            two_hop_preds.append(pred)

    triples_list: List = list()

    for entity in entities:
        one_hop_query = build_sparql_query_for_one_hop_predicates(entity, one_hop_preds)
        # two_hop_query = build_sparql_query_for_two_hop_predicates(entity, two_hop_preds)

        graph_first_query = query_dbpedia(one_hop_query, RDFXML)
        # graph_second_query = query_dbpedia(two_hop_query, RDFXML)

        # graph_query = graph_first_query + graph_second_query
        graph_query = graph_first_query
        triples_list = add_new_triples_without_duplicates_to_triples_list(
            triples_list, graph_query
        )
...
def triples_for_predicates_all_datasets(
    ...
    first_pred = 0
    last_pred = 69
    while len(final_triples_list) < number_of_triples and num_of_query < 1:  # 11:
        triples_list = query_dbpedia_for_all_entities(
            entities, predicates_table
...
def main() -> None:
    ...
    triples = triples_for_predicates_all_datasets(
        "How large is the area of UK?", "./pickle_objects/qald9_qald8_lcquad.pickle"
    )
    print(len(triples))
