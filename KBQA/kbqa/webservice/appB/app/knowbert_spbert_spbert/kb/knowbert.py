...
from app.knowbert_spbert_spbert.kb.common import extend_attention_mask_for_bert
from app.knowbert_spbert_spbert.kb.common import get_dtype_for_module
from app.knowbert_spbert_spbert.kb.common import init_bert_weights
from app.knowbert_spbert_spbert.kb.common import set_requires_grad
from app.knowbert_spbert_spbert.kb.entity_linking import BaseEntityDisambiguator
from app.knowbert_spbert_spbert.kb.entity_linking import EntityLinkingBase
from app.knowbert_spbert_spbert.kb.evaluation.exponential_average_metric import ExponentialMovingAverage
from app.knowbert_spbert_spbert.kb.evaluation.weighted_average import WeightedAverage
from app.knowbert_spbert_spbert.kb.metrics import MeanReciprocalRank
from app.knowbert_spbert_spbert.kb.span_attention_layer import SpanAttentionLayer
from app.knowbert_spbert_spbert.kb.wordnet import WordNetAllEmbedding
import numpy as np
...
class BertPretrainedMetricsLoss(Model):
    def __init__(self, vocab: Vocabulary, regularizer: RegularizerApplicator = None):
    ...
    def _compute_loss(
        ...
        if update_metrics:
            ...
            for keys, v in [
                [["total_loss_ema", "total_loss"], total_loss.item()]
            ] + loss_metrics:
                for key in keys:
                    self._metrics[key](v)
...
class DotAttentionWithPrior(torch.nn.Module):
    ...
    def _get_weighted_entity_embeddings(
        ...
        weighted_entity_embeddings = (
            normed_linking_scores.unsqueeze(-1) * candidate_entity_embeddings
        ).sum(dim=2)
...
class EntityDisambiguator(BaseEntityDisambiguator, torch.nn.Module):
    def __init__(
        ...
        self.projected_span_layer_norm = torch.nn.LayerNorm(
            entity_embedding_dim, eps=1e-5
        )
        init_bert_weights(self.projected_span_layer_norm, initializer_range)

        self.kg_layer_norm = torch.nn.LayerNorm(entity_embedding_dim, eps=1e-5)
        ...
        # layers for the dot product attention
        if (
            weighted_entity_threshold is not None
            or include_null_embedding_in_dot_attention
        ):
    ...
    def _run_span_encoders(
        ...
        return self.span_encoder(x, attention_mask, output_all_encoded_layers=False)
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        knowbert_logger.debug(
            f"candidate_entity_embeddings: {candidate_entity_embeddings}"
        )
        ...
        projected_span_representations = self.span_extractor(
            projected_bert_representations, candidate_spans, mask, span_mask
        )
...
class EntityLinkingWithCandidateMentions(EntityLinkingBase):
    def __init__(
        ...
        num_embeddings_passed = sum(
            [
                kg_model is not None,
                entity_embedding is not None,
                concat_entity_embedder is not None,
            ]
        )
        if num_embeddings_passed != 1:
            raise ValueError(
                "Linking model needs either a kg factorisation model or an entity embedding."
            )
...
class SolderedKG(Model):
    def __init__(
        ...
        self.weighted_entity_layer_norm = torch.nn.LayerNorm(
    ...
    def _init_kg_to_bert_projection(self) -> None:
        ...
        w = (
            self.entity_linker.disambiguator.bert_to_kg_projector.weight.data.detach().numpy()
        )
        w_pseudo_inv = np.dot(np.linalg.inv(np.dot(w.T, w)), w.T)
        b = (
            self.entity_linker.disambiguator.bert_to_kg_projector.bias.data.detach().numpy()
        )
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        spans_with_entities = self.weighted_entity_layer_norm(
            (
                span_representations + self.dropout(weighted_entity_embeddings)
            ).contiguous()
        )
        ...
        new_contextual_embeddings = self.output_layer_norm(
            (
                contextual_embeddings + self.dropout(bert_representations_with_entities)
            ).contiguous()
        )
...
class KnowBert(BertPretrainedMetricsLoss):
    def __init__(
        ...
        if remap_segment_embeddings is not None:
            ...
            new_embeddings = self._remap_embeddings(
                self.pretrained_bert.bert.embeddings.token_type_embeddings.weight
            )
    ...
    def load_state_dict(self, state_dict: Dict, strict: bool = True) -> None:
        if self.remap_segment_embeddings:
            ...
            new_embeddings = self._remap_embeddings(
                state_dict[
                    "pretrained_bert.bert.embeddings.token_type_embeddings.weight"
                ]
            )
            if new_embeddings is not None:
                state_dict[
        ...
        for layer_num, soldered_kg_key in self.layer_to_soldered_kg:
            ...
            if end_layer_index > start_layer_index:
                ...
                for layer in self.pretrained_bert.bert.encoder.layer[
                    ...
                    knowbert_logger.debug(
                        f"Bert Layer{layer_num},{layer_idx}: {contextual_embeddings}"
                    )
                    layer_idx += 1
            start_layer_index = end_layer_index
