...
from app.summarizer.entity_relation_hops import entity_relation_hops
from app.summarizer.entity_relation_hops import subgraphs_on_relations
from app.summarizer.ranking import get_ranking_tables
from app.summarizer.ranking import rank_list
...
class OneHopRankSummarizer(Summarizer):
    ...
    PRINT = False
    DATASET_PATH = "app/summarizer/datasets/"
    EXCLUDE = [
    ...
    def summarize(self, question: str) -> List[str]:
        ...
        print(
            "Recognized entities:", [str(entity).encode("utf-8") for entity in entities]
        )
...
def subgraphs_for_entities(
    ...
    for entity in entities:
        reg_graph, _ = subgraphs_on_relations(entity, reg_predicates)
        _, inv_graph = subgraphs_on_relations(entity, inv_predicates)
