...
class QA_query_generation:
    ...
    def get_query(self, nl_question):
        ...
        for word in sentence.words:
            print(
                f"{word.text}\t{word.lemma}\t{word.pos}\t{word.head}\t"
        ...
        considered_words = [
            word for word in sentence.words if word.pos in considered_POS_tags
        ]
        # Remove duplicates
        ...
        for word_permutations in itertools.permutations(considered_word_lemmas, 2):
            ...
            subj_query = query_string.format(
                "?s",
                "?s",
                "<" + self._dbpedia_resource_base + word0 + ">",
                "<" + self._dbpedia_ontology_base + word1 + ">",
            )
            yield subj_query
            obj_query = query_string.format(
                "?o",
                "<" + self._dbpedia_resource_base + word0 + ">",
                "<" + self._dbpedia_ontology_base + word1 + ">",
                "?o",
