...
def convert_dataset(dataset: dict, start_offset: int) -> dict:
    """
    Convert QALD dataset to QTQ dataset.

    Iterates through all questions in QALD datasets.

    :param dataset: QALD dataset
    :param start_offset: Offset into dataset for skipping questions
    :return: QTQ dataset
    """
    result_dataset: Dict = {"questions": []}
    q_cnt = start_offset
    for question in dataset["questions"][start_offset:]:
        ...
        print(f"Question #{q_cnt}: {nl_question}")
        ...
        if len(relevant_triples) == 0:
            ...
            print(f'SPARQL query: {question["query"]["sparql"]}')
            ...
            result_dataset["questions"].append(qtq_dict)
...
def main() -> None:
    """
    Load QALD dataset, processes it and store qtq dataset.
    ...
    try:
        ...
        print("Specifiy QALD file with --dataset=path_to_file")
        sys.exit()

    result_dataset = convert_dataset(qald_data, start_offset)

    with open(
        dataset_path.replace("qald", "qtq"), "w", encoding="utf-8"
    ) as file_handle:
        json.dump(result_dataset, file_handle, indent=4, separators=(",", ": "))
