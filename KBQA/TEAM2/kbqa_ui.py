...
def findTheAnswer():
    ...
    if questionEntryValue.get() != "":
        ...
        answer = Label(
            root,
            text="Answer is :  " + answerValue.get(),
            bg="lightblue",
            font=("Times", 13, "bold"),
        ).place(x=300, y=220)
    else:
...
title = Label(
    root, text="Question Answering System", bg="lightblue", font=("Impact", 25, "bold")
).place(x=430, y=30)

# Question
question = Label(
    root, text="Enter your question : ", bg="lightblue", font=("Times", 14, "bold")
).place(x=300, y=100)
...
# Button
Button(
    text="Find the answer", command=findTheAnswer, bd="1", font=("Times", 13, "bold")
).place(x=490, y=160)
